...
void init_win() {
  ...
  const char* info_text = " Exit: ESC | Reload: R | New Search: BACKSPACE | Print Help: "
  ...
  wrefresh(info_window);
    ...
    WINDOW* border_window = subwin(stdscr, rows-1, (cols-1) / 2, 1, 0);
  ...
  scr = subwin(border_window, rows-3, (cols-1)/2 - 3, 2, 2);
  ...
  list_scr = subwin(border_window, rows-3, (cols-1)/2 - 3, 2, cols / 2 - 1 + 2);
  max_per_page = rows-3;
}
